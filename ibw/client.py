...
class IBClient():
    ...
    def __init__(self, username: str, account: str, client_gateway_path: str = None, is_server_running: bool = True) -> None:
        #Changed the "client_gatewat_path: str = None" to "client_gatewat_path: str = gateway_path " where "gateway_path = pathlib.Path('clientportal.gw').resolve()" as defined above
    ...
    def get_order_status(self,trade_id:str):
        """
            The end-point is meant to be used in polling mode, e.g. requesting every 
            x seconds. It queries the status of a live order with IB's order_id.
            The response will contain a list of informaiton about the order. 
        """

        # define request components
        endpoint = r'iserver/account/order/status/{}'.format(trade_id)
        req_type = 'GET'
        content = self._make_request(
            endpoint=endpoint,
            req_type=req_type
